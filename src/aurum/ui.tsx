...
export const Toast = () => {
  const { toastMsg, G } = useAurum();
  if (!toastMsg) return null;
  return (
    <div style={{ position: "fixed", bottom: 90, left: "50%", transform: "translateX(-50%)", background: G.card, border: `1px solid ${G.gold}`, color: G.text, padding: "12px 20px", borderRadius: 12, fontSize: 14, zIndex: 1000, boxShadow: "0 8px 30px rgba(0,0,0,0.5)", maxWidth: 320, textAlign: "center" }}>{toastMsg}</div>
  );
};

export function ScreenShell({ title, children, onBack }: { title: string; children: ReactNode; onBack: () => void }) {
  const { s, G } = useAurum();
  return (
    <div style={{ ...s.scroll, position: "relative" }}>
      ...
      <div style={{ padding: "20px 22px 100px", position: "relative" }}>
        ...
        <h1 style={{ ...s.serif, fontSize: 28, margin: "20px 0 8px", fontWeight: 600 }}>{title}</h1>
        {children}
      </div>
    </div>
  );
